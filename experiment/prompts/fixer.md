...
Your deadline is `{{WALL_CLOCK_DEADLINE_ISO}}` (25 minutes maximum). There is no enforceable token ceiling; usage must be recorded if exposed. Run appropriate checks, commit once, and produce a fix artifact conforming to `experiment/schemas/fix.schema.json`. Stop after this one agent/subagent turn.
