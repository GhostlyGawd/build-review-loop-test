...
`golden-run.json` is synthetic protocol evidence used only to test execution-mode validation. It demonstrates a valid completed run that preserves one historical invalid rehearsal without candidate contents. `invalid-current.json` demonstrates a currently invalid run with structured evidence commitments and no evaluation conclusion or scored outcome.
