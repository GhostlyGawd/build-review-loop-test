...
Protocol version: 2.7.0 (locked actual-exec permission repair)
...
This Markdown protocol is the canonical public source. `experiment/canonical-contract.json` is its machine-readable transcription and is content-addressed in `experiment/lock.json`. A mismatch invalidates the scaffold. Protocol 2.7.0 repairs the actual-exec permission defect discovered under the 2.6.0 lock at `a45a257a1039c6de2729d23fff2ef427ce59e784`. Two fresh neutral builders ran the exact frozen `codex exec` command concurrently and exited zero, but the model sessions were read-only: `apply_patch` and npm execution were blocked, no worktree changed, and no supervisor child commit was created. The lower-level no-model `codex sandbox` probe had passed but did not attest actual model-session permissions. Assignment seed generation, assignment, treatment, testing, evaluation, outcome selection, and hidden-suite access did not occur. The sanitized record is `experiment/preflight/aborted-lock-2.6.0.json`; it binds the prompt and runtime-contract commitments plus prefixes of the separately retained private evidence hashes. Protocol 2.7.0 preserves the neutral candidate projection bytes, task, rubric, hidden-suite commitment, public gates, `-a never`, and post-build randomization. It adds an explicit `windows.sandbox=\"elevated\"` override, an explicit `sandbox_workspace_write.network_access=false` override, `--ignore-rules`, and a harmless actual `codex exec` gate using the same permission-relevant builder configuration, environment-key behavior, and three add-dir roles. The gate verifies ordinary model-session workspace/add-dir writes, protected Git metadata, unchanged private/outside canaries, and a clean hook-free supervisor child commit. Its prompt requests Git/Node/npm checks, but the retained attestation does not independently bind their command events or outputs. The older lower-level probe remains secondary evidence only. Before builder exposure, the final lock MUST:
...
9. record the sanitized prior gates and both permission probes, require the actual-exec attestation as authoritative, prove its permission-relevant argv/environment/add-dir dimensions equal the builder runner, and confirm no replacement builder has received task materials after the 2.6 abort.
...
The committed source-parity inventory records exact portable-skill parity for protocol-derived bytes. The neutral projection allowlist, projected package bytes, public tests, product specification, rubric, hidden-suite commitment, and public-gate set remain unchanged. The execution repair is limited to runtime configuration, permission probes, runners, contracts, schemas, validation, skill packaging, sanitized abort/preflight records, and the final lock.
