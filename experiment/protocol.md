...
The steward invokes `scripts/run-cli-builders.ps1 -ContractPath <absolute-contract.json>`. The contract supplies two independent full clones and evidence destinations. Both remotes are disabled, both clones are clean at the exact common-start commit/tree, their opaque paths are nonnested, and they run concurrently. Each gets distinct temp/cache/dependency roots, port, and evidence directory outside either clone. The executable, version, binary hash, ChatGPT auth, `--ignore-user-config`, and invariant argv are identical. The exact argv begins `-a never -m gpt-5.4 -c model_reasoning_effort="xhigh" exec --ephemeral --ignore-user-config --skip-git-repo-check --sandbox workspace-write --json`; only `-C <workdir>` and `-o <final-path>` differ, followed by `-`.
...
Every model-bearing role—builder, adversarial reviewer, fixer, tester, and blinded evaluator—runs through `scripts/run-cli-builders.ps1` or `scripts/run-cli-role.ps1` under the frozen lock/schema/runtime rules. Each gets exactly one ephemeral turn, a distinct invocation/process/thread identity, exact role-prompt bytes on stdin, and no resume or inherited transcript. The single-role contract binds the lock, role runner, supervision schema, role prompt template, deterministic substitutions, rendered prompt, handoff where applicable, role artifact schema, input commit/tree, and fresh evidence destinations. Evaluator contracts additionally bind the private blinded-package manifest/schema/packager, label, package-content hash, rubric, and hidden suite. Reviewer inputs are read-only snapshots whose HEAD/tree must remain identical. Fixers use authorized isolated full-clone writes, must finish clean, and bind exactly one child commit to the input. Testers and evaluators use disposable workspace-write copies whose input commit/tree or package hash must remain unchanged. The runner injects the observed invocation/process/thread identity into the final role artifact before schema validation; the supervision record separately binds the contract and final artifact hashes, avoiding a circular self-hash. Token ceilings are unavailable and remain `null`; `turn.completed.usage` is captured when exposed and otherwise has an explicit absence reason.
...
Invoke `node scripts/package-blinded-snapshots.mjs --mapping <private-mapping.json> --output-root <fresh-output-directory> --manifest <private-manifest.json>`. The private mapping lists exactly X/Y/Z in randomized order and exactly one B0/T0/Tfinal clean full-clone source with its Git commit/tree plus the frozen public-gate-set hash. The script rejects path traversal, nested sources/outputs/manifests, source drift, symlinks, lineage tokens, and a reused output root. It strips Git history, experiment/role evidence, generated dependencies/output, timestamps, and candidate/worker identifiers; normalizes timestamps; and hashes UTF-8-sorted package contents into a private manifest outside the output root. Blinding is mapping blinding, not a claim that lineage is intrinsically unknowable. A fresh evaluator subprocess sees exactly one history-free X, Y, or Z package, finishes its score, then records a mapping guess, integer confidence percentage from 0 through 100, and observable diagnostic evidence. It seals the artifact before the next evaluator launches; revisions are prohibited. Each evaluator verifies suite commitments, runs the frozen public and sealed hidden suites in an equivalent disposable Node 22 copy, and emits one evaluation artifact. Only after all three seals may the operator reveal the mapping and compute the registered deltas.
