...
Protocol version: 2.8.0 (locked supervisor-evidence repair)
...
This Markdown protocol is the canonical public source. `experiment/canonical-contract.json` is its machine-readable transcription and is content-addressed in `experiment/lock.json`. A mismatch invalidates the scaffold. Protocol 2.8.0 repairs the supervisor evidence-binding defect discovered under the 2.7.0 lock at `e2c892e3b88702fcbd9a2049926d4a28b94efa4a`. Two fresh neutral builders ran concurrently under the exact frozen runtime, exited zero, and each received one clean hook-free supervisor child commit. The frozen runner then piped the helper JSON into a function whose parameter did not accept pipeline input, so both supervision records stored null commit bindings and construction was correctly invalidated before assignment. The same deterministic parsing defect existed in the fixer handoff. Assignment seed generation, assignment, treatment, testing, evaluation, outcome selection, and hidden-suite access did not occur. The sanitized record is `experiment/preflight/aborted-lock-2.7.0.json`; it binds only runtime and private-evidence commitments. Protocol 2.8.0 preserves the neutral candidate projection bytes, task, rubric, hidden-suite commitment, five-stage public gate, permission configuration and actual-exec attestation, `-a never`, and post-build randomization. It parses supervisor helper output explicitly and fails closed on nonzero, empty, malformed, null, or multiple-object results, with PowerShell integration regressions for builder and fixer paths. Before builder exposure, the final lock MUST:
...
9. record the sanitized prior gates, the 2.7 invalidation, and both permission probes; require the actual-exec attestation as authoritative; prove its permission-relevant argv/environment/add-dir dimensions equal the builder runner; and confirm no replacement builder has received task materials after the 2.7 abort.
...
The committed source-parity inventory records exact portable-skill parity for protocol-derived bytes. The neutral projection allowlist, projected package bytes, public tests, product specification, rubric, hidden-suite commitment, five-stage public-gate set, and actual-exec permission attestation remain unchanged. The repair is limited to supervisor helper parsing, validation, skill packaging, sanitized abort/preflight records, and the final lock.
