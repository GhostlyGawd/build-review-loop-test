...
description: Run or audit the provisional protocol-v2.8 neutral-build versus treatment-only review-loop pilot with pinned external Codex CLI subprocesses, an attested Windows actual-exec workspace-write boundary, fail-closed supervisor commit evidence, post-build random assignment, bounded fresh reviewer/fixer/tester roles, canonical evidence, and mapping-blinded B0/T0/Tfinal evaluation. Use when executing, validating, packaging, or diagnosing this exact public experiment without changing its frozen runtime or evaluation design.
---
...
3. Require the frozen PowerShell 7 host and invoke each runner with `-NoProfile -File`. Require the frozen Codex binary, ChatGPT authentication, `gpt-5.4`, `xhigh`, exact argv order, `--ephemeral`, `--ignore-user-config`, `--ignore-rules`, explicit `windows.sandbox=\"elevated\"`, explicit disabled workspace-write network, and no resume.
...
From one clean exact source commit/tree, run [prepare-builder-input.mjs](scripts/prepare-builder-input.mjs) with the frozen [builder-input-allowlist.json](references/builder-input-allowlist.json). Require two identical source-history-free one-root projections and a private schema-valid manifest outside both workdirs. Builders never receive source history, protocol material, or the private manifest.
...
Before exposing any experiment task material, require the committed harmless actual-exec attestation produced by [verify-cli-exec-permissions.ps1](scripts/verify-cli-exec-permissions.ps1) from [actual-exec-capability-probe.md](references/actual-exec-capability-probe.md). Its invariant argv, environment-key set, and three add-dir roles must equal the builder runner on every permission-relevant dimension. Require actual model-session workspace/add-dir writes, unchanged `.git` before the supervisor, unchanged private/outside canaries, one completed thread/turn, exit zero, and a clean hook-free one-child supervisor commit. The prompt requests Git/Node/npm checks, but do not treat them as independently attested without bound command events and outputs. Treat [permission-probe-2.6.0.json](references/permission-probe-2.6.0.json) and [verify-cli-permissions.ps1](scripts/verify-cli-permissions.ps1) as secondary lower-level evidence only.

Use the byte-exact [run-cli-builders.ps1](scripts/run-cli-builders.ps1) through the pinned `pwsh -NoProfile -File` host. It validates contract/schema/lock/helper/runner/binary/auth/projection/path bindings and passes only the three exact external runtime roots through `--add-dir`. Models may edit authorized worktree files but must not modify `.git` or commit. After valid lifecycle evidence, [commit-candidate.ps1](scripts/commit-candidate.ps1) rejects frozen-path drift and creates exactly one hook-free child commit through an external index. Require the runner to parse exactly one helper JSON object and fail closed on nonzero, empty, malformed, null, or multiple-object output. Accept only distinct process and `thread.started` IDs, one `turn.completed`, reconciled hashes, exit zero, exact closed evidence trees, and clean one-child projections.
...
The packager must reject traversal, nesting, source/ref/exact-HEAD/tree drift, non-clean or autocrlf-mutated sources, symlinks or reparse/short-name aliases, frozen-gate drift, lineage tokens, and reused outputs. Its evaluator-facing package manifests contain only labels and package seals—never source role/ref/commit/tree/path or private provenance.
